...
class CargaisonFetcher {
    ...
    async displayCargaisons() {
        this.filteredData = await this.fetchCargaisons();
        this.currentPage = 1; // Reset à la première page
        this.displayCurrentPage();
    }
    async filtrerParStatus() {
        const cargaisons = await this.fetchCargaisons();
        const typeFilter = document.querySelector("#type-filter")?.value;
        const avancementFilter = document.querySelector("#etat-filter")?.value;
        const globalFilter = document.querySelector("#global-filter")?.value;
    ...
    async updateStats() {
        ...
        if (elements.ouvertes)
            elements.ouvertes.textContent = ouvertes.toString();
        if (elements.enCours)
            elements.enCours.textContent = enCours.toString();
        if (elements.fermees)
            elements.fermees.textContent = fermees.toString();
    }
}
